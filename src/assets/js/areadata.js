...
strVarCity += "                          <div class=\"data-result\"><em>最多只能选择<strong>10</strong>个城市<\/em><\/div>";
...
/*if(wx.getstro.... != ''){
   var obj = wx.getStor(member[i].name+myname)[wx.getst(member[i].name+myname).length-1];
   obj['你要改或者加的字段'] = 你要改的值;
   array.push(obj)
}*/
    ...
    Vue.prototype.selectProvince('all', null, '');
}


var dataarrary = Vue.__LocalDataCities.list;
Vue.prototype.selectProvince = function(type, con, isremove) {
    console.log('显示内容了')
    ...
    var strVarCity = "";
    if (type == "all") {
        var dataCityxz = Vue.__LocalDataCities.category.provinces;
        var datahotcityxz = Vue.__LocalDataCities.category.hotcities;
        ...
        for (var i in datahotcityxz) {
            strVarCity += '<li><a href=\"javascript:;\" data-code=\"' + datahotcityxz[i] + '\" data-name=\"' + dataarrary[datahotcityxz[i]][0] + '\" class=\"d-item\"  onclick=self.selectProvince(\'sub\',this,\'\')>' + dataarrary[datahotcityxz[i]][0] + '<label>0</label></a></li>';
        }
        strVarCity += "                                <\/ul>";
        strVarCity += "                              <\/div>";
        ...
        for (var i in dataCityxz) {
            strVarCity += '<li><a href=\"javascript:;\" data-code=\"' + dataCityxz[i] + '\" data-name=\"' + dataarrary[dataCityxz[i]][0] + '\" class=\"d-item\"  onclick="self.selectProvince(\'sub\',this,\'\')">' + dataarrary[dataCityxz[i]][0] + '<label>0</label></a></li>';
        }
        strVarCity += "                                <\/ul>";
        strVarCity += "                              <\/div>";
    ...
    //显示下一级
        ...
        var dataCityxz = Vue.__LocalDataCities.category.provinces;
        var datahotcityxz = Vue.__LocalDataCities.category.hotcities;
        var relations = Vue.__LocalDataCities.relations;
            console.log(relations[$(con).data("code")]);
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            console.log(datahotcityxz.indexOf(relations[$(con).data("code")]))
            //添加标题
            if (isremove != "remove") {
            ...
            //添加内容
             ...
             for (var i in relations[$(con).data("code")]) {
                 strVarCity += '<li><a href="javascript:;" class="d-item" data-code="' + relations[$(con).data("code")][i] + '"  data-name=\"' + dataarrary[relations[$(con).data("code")][i]][0] + '\" onclick="Vue.prototype.selectProvince(\'sub\',this,\'\')">' + dataarrary[relations[$(con).data("code")][i]][0] + '<label>0</label></a></li>';
             }
             strVarCity += "<\/ul>";
            $('.data-container-city').html(strVarCity);
        } else {
    ...
    if (datatype == "duoxuan") {
        ...
        if ($('.data-result span').length > 5) {
            ...
            if ($(con).attr("class").indexOf("d-item-active") == -1) {
                ...
                $(con).parent('li').nextAll("li").find('a').attr("onclick", "Vue.prototype.selectProvince(\'sub\',this,\'\')");
            }
            if ($(con).attr("class").indexOf('d-item-active') > 0) {
                $('.data-result span[data-code="' + $(con).data("code") + '"]').remove();
                $(con).removeClass('d-item-active');
...
function removenode_area(lithis) {
    ...
    if ($(lithis).data("selector") == "tab-all") {
        ...
        Vue.prototype.selectProvince('sub', lithis, 'remove');
    }
}
...
function removespan_area(spanthis) {
    ...
    if ($('a[data-code=' + $(spanthis).data("code") + ']').length > 0) {
        if ($('a[data-code=' + $(spanthis).data("code") + ']').attr("class").indexOf('data-all') > 0) {
            ...
            $('a[data-code=' + $(spanthis).data("code") + ']').parent('li').nextAll("li").find('a').attr("onclick", "Vue.prototype.selectProvince(\'sub\',this,\'\')");
...
Vue.cityShow = function(thiscon, Cityxz){
    appendCity(thiscon, Cityxz);
};

/*function aaa(n){
    alert(n);
}
Vue.cityShow = function(n){
    aaa(n)
}*/
 }//install尾部
}
